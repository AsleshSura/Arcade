...
def _btn(parent, text, command, color=ACCENT, fg=TEXT, font=None, **kw):
    """Label-based button — bg color is respected on all platforms."""
    frame = tk.Frame(parent, bg=color, cursor="hand2")
    lbl = tk.Label(
        frame, text=text, bg=color, fg=fg,
        font=font or F_BTN, cursor="hand2",
        padx=16, pady=9,
    )
...
class GameManager(tk.Tk):
    ...
    def _build_header(self):
        ...
        self._scan_frame = tk.Frame(hdr, bg=MUTED, cursor="hand2")
        self._scan_frame.pack(side=tk.RIGHT, padx=18, pady=10)
        self._scan_lbl = tk.Label(
            self._scan_frame, textvariable=self._scan_btn_text,
            bg=MUTED, fg=TEXT, font=F_BTN_SM,
            cursor="hand2", padx=12, pady=6,
        )
        self._scan_lbl.pack()
        self._scan_lbl.bind("<Button-1>", lambda e: self._toggle_scanner())
        self._scan_frame.bind("<Button-1>", lambda e: self._toggle_scanner())

    def _toggle_scanner(self):
        if self.nfc.active:
            self.nfc.stop()
            self._scan_btn_text.set("Scanner: OFF  ○")
            self._scan_frame.config(bg=MUTED)
            self._scan_lbl.config(bg=MUTED)
        else:
            self.nfc.start()
            self._scan_btn_text.set("Scanner: ON  ●")
            self._scan_frame.config(bg=GREEN)
            self._scan_lbl.config(bg=GREEN)
    ...
    def _make_game_card(self, parent, title, emoji, players_text,
        ...
        if enabled:
            _btn(c, "Select", on_select, color=ACCENT, font=F_BTN_SM).pack(pady=10)
        else:
            _lbl(c, disabled_hint, color=ACCENT, bg=CARD_BG, font=F_MUTED).pack(pady=(6, 0))
            tk.Label(
                c, text="Select", bg=MUTED, fg=CARD_BG,
                font=F_BTN_SM, padx=12, pady=6,
            ).pack(pady=4)
        return c
